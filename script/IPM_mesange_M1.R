...
code <- nimbleCode({
  ...
  for(t in 2:K){
    log(fec_bt[1,t]) <- mu.fec_bt[1] + DDIA_fec_bt[1] * (Nad_bt[1,t]/2000) + eps.fec_bt[1,t]
    log(fec_bt[2,t]) <- mu.fec_bt[2] + DDIA_fec_bt[2] * (Nad_bt[2,t]/4000) + eps.fec_bt[2,t]
    #Random effect time
  ...
  for (t in 1:K){ 
    ...
    # Survival
    logit(eta.phi_bt[1,1,t]) <- mu.phi_bt[1,1] + DDIA_phiJ_bt[1] * (Ntot_bt[1,t]/10000) + eps.phi_bt[1,1,t]
    logit(eta.phi_bt[1,2,t]) <- mu.phi_bt[1,2] + eps.phi_bt[1,2,t]
    logit(eta.phi_bt[2,1,t]) <- mu.phi_bt[2,1] + DDIA_phiJ_bt[2] * (Ntot_bt[2,t]/19000) + eps.phi_bt[2,1,t]
  ...
  for(t in 2:K){
    log(fec_gt[1,t]) <- mu.fec_gt[1] + DDIA_fec_gt[1] * (Nad_gt[1,t]/3000) + eps.fec_gt[1,t]
    log(fec_gt[2,t]) <- mu.fec_gt[2] + DDIA_fec_gt[2] * (Nad_gt[2,t]/5000) + eps.fec_gt[2,t]
    #Random effect time
  ...
  for (t in 1:K){
    ...
    # Survival
    logit(eta.phi_gt[1,1,t]) <- mu.phi_gt[1,1] + DDIA_phiJ_gt[1] * (Ntot_gt[1,t]/11000) + eps.phi_gt[1,1,t]
    logit(eta.phi_gt[1,2,t]) <- mu.phi_gt[1,2] + eps.phi_gt[1,2,t]
    logit(eta.phi_gt[2,1,t]) <- mu.phi_gt[2,1] + DDIA_phiJ_gt[2] * (Ntot_gt[2,t]/22000) + eps.phi_gt[2,1,t]
...
parameters <- c("eta.phi_bt","eta.p_bt","fec_bt",
                ...
                'Nad_ts',"Ntot_ts",
                "DDIA_phiJ_gt", "DDIA_phiJ_bt","DDIA_fec_gt", "DDIA_fec_bt")
...
save(out,file='out_tits_M1.RData')
