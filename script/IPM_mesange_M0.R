...
code <- nimbleCode({
  ...
  for(t in 2:K){
    log(fec_bt[1,t]) <- mu.fec_bt[1] + eps.fec_bt[1,t]
    log(fec_bt[2,t]) <- mu.fec_bt[2] + eps.fec_bt[2,t]
    #Random effect time
  ...
  for (t in 1:K){ 
    ...
    # Survival
    logit(eta.phi_bt[1,1,t]) <- mu.phi_bt[1,1] + eps.phi_bt[1,1,t]
    logit(eta.phi_bt[1,2,t]) <- mu.phi_bt[1,2] + eps.phi_bt[1,2,t]
    logit(eta.phi_bt[2,1,t]) <- mu.phi_bt[2,1] + eps.phi_bt[2,1,t]
  ...
  for(t in 2:K){
    log(fec_gt[1,t]) <- mu.fec_gt[1] + eps.fec_gt[1,t]
    log(fec_gt[2,t]) <- mu.fec_gt[2] + eps.fec_gt[2,t]
    #Random effect time
  ...
  for (t in 1:K){
    ...
    # Survival
    logit(eta.phi_gt[1,1,t]) <- mu.phi_gt[1,1] + eps.phi_gt[1,1,t]
    logit(eta.phi_gt[1,2,t]) <- mu.phi_gt[1,2] + eps.phi_gt[1,2,t]
    logit(eta.phi_gt[2,1,t]) <- mu.phi_gt[2,1] + eps.phi_gt[2,1,t]
...
save(out,file='out_tits_M0.RData')
