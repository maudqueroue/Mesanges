...
load(here::here('output','hvie_sylatr_nord.RData'))
...
rm(ID_sylatr,hvie_sylatr,data)

data_ind <- data_sylatr
...
data_annee_ref <- subset(data, annee=="2019")
...
for (i in 1:Nombre_annees) {
  data_annee_test <- subset(data, annee==annees[i])
  ...
  for (h in 1:l){
    ...
    model <- gamm4(n~s(date),family=poisson,data=data_annee)
