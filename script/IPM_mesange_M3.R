...
code <- nimbleCode({
  ...
  for(t in 2:K){
    log(fec_bt[1,t]) <- mu.fec_bt[1] + DDT_fec_bt[1] * (Nad_ts[1,t]/5000) + eps.fec_bt[1,t]
    log(fec_bt[2,t]) <- mu.fec_bt[2] + DDT_fec_bt[2] * (Nad_ts[2,t]/9000) + eps.fec_bt[2,t]
    #Random effect time
  ...
  for (t in 1:K){ 
    ...
    # Survival
    logit(eta.phi_bt[1,1,t]) <- mu.phi_bt[1,1] + DDT_phiJ_bt[1] * (Ntot_ts[1,t]/21000) + eps.phi_bt[1,1,t]
    logit(eta.phi_bt[1,2,t]) <- mu.phi_bt[1,2] + eps.phi_bt[1,2,t]
    logit(eta.phi_bt[2,1,t]) <- mu.phi_bt[2,1] + DDT_phiJ_bt[2] * (Ntot_ts[2,t]/40000) + eps.phi_bt[2,1,t]
  ...
  for(t in 2:K){
    log(fec_gt[1,t]) <- mu.fec_gt[1] + DDT_fec_gt[1] * (Nad_ts[1,t]/5000) + eps.fec_gt[1,t]
    log(fec_gt[2,t]) <- mu.fec_gt[2] + DDT_fec_gt[2] * (Nad_ts[2,t]/9000) + eps.fec_gt[2,t]
    #Random effect time
  ...
  for (t in 1:K){
    ...
    # Survival
    logit(eta.phi_gt[1,1,t]) <- mu.phi_gt[1,1] + DDT_phiJ_gt[1] * (Ntot_ts[1,t]/21000) + eps.phi_gt[1,1,t]
    logit(eta.phi_gt[1,2,t]) <- mu.phi_gt[1,2] + eps.phi_gt[1,2,t]
    logit(eta.phi_gt[2,1,t]) <- mu.phi_gt[2,1] + DDT_phiJ_gt[2] * (Ntot_ts[2,t]/40000) + eps.phi_gt[2,1,t]
  ...
  for(h in 1:2){
    #bt
    DDT_phiJ_bt[h] ~ dnorm(0, sd=2)
    DDT_fec_bt[h] ~ dnorm(0, sd=0.5)
    #gt
    DDT_phiJ_gt[h] ~ dnorm(0, sd=2)
    DDT_fec_gt[h] ~ dnorm(0, sd=0.5)
  }#h
...
parameters <- c("eta.phi_bt","eta.p_bt","fec_bt",
                ...
                'Nad_ts',"Ntot_ts",
                "DDT_phiJ_gt", "DDT_phiJ_bt","DDT_fec_gt", "DDT_fec_bt")
...
save(out,file='out_tits_M3.RData')
