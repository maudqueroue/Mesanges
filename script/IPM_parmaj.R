...
load('index_parmaj_hab1_sd.RData')
load('index_parmaj_hab2_mean.RData')
load('index_parmaj_hab2_sd.RData')
...
sd.counts[1,2:19] <- index_parmaj_hab1_sd
sd.counts[2,2:19] <- index_parmaj_hab2_sd

rm(index_parmaj_hab1_mean,index_parmaj_hab1_sd,index_parmaj_hab2_mean,index_parmaj_hab2_sd,hvie_ID_PROG_parmaj,hvie_parmaj)
...
code <- nimbleCode({
  ...
  for(h in 1:2){
    ...
    #Juveniles
    nN1[h] ~ dnorm(200, sd = 20)
    N1[h,1] <- round(nN1[h])
    
    # Adults
    nNad[h] ~ dnorm(100, sd = 20)
...
init1  <- list(z=cjs.init.z(mydata,f),
               ...
               Nad = matrix(rep(c(75,120), 19, each=1),2,19))
...
init2  <- list(z=cjs.init.z(mydata,f),
               ...
               gamma.u.phi=rnorm(2,0,1),
               gamma.i.p=rnorm(1,0,1),
               Nad = matrix(rep(c(60,100), 19, each=1),2,19))
...
# library(boot)
# 
# setwd("~/These/MNHN/CMR_parmaj/IPM_parmaj_4")
# 
# # Data
# load('hvie_parmaj_tot.RData')
# 
