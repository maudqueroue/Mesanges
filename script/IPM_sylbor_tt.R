...
#setwd("//hpcm.cluster.calcul.hpc/Shared-2/QUEROUE/IPM_sylbor_tt")
#setwd("~/These/MNHN/Mesanges_cluster/IPM_sylbor_tt")
...
# Log habitat
load('index_sylbor_mean.RData')
load('index_sylbor_var.RData')

counts <- NULL
counts[2:19] <- index_sylbor_mean

sd.counts <- NULL
sd.counts[2:19] <- sqrt(index_sylbor_var)
...
rm(index_sylbor_mean,index_sylbor_var,i,hvie_ID_PROG_sylbor,hvie_sylbor,S,j,temp)
...
code <- nimbleCode({
  ...
  #Juveniles
  nN1 ~ dnorm(250, sd = 150)

  # Adults
  nNad ~ dnorm(150, sd = 80)
...
save(out,file='out_IPM_sylbor.RData')
...
color<-c("#FF8830","#A6B06D","#589482","#8C2423")
par(mfrow=c(2,2))
plot_states("N1[", 1, color[1],c(0,1000), "annees","nombre","Nb juv")
check_state("Nad[", exp(index_sylbor_mean), col = color[2], x_lab="annees", y_lab="nombre",title="Nb Ad")
...
plot_states("Nad_juv[", 2, color[1],c(0,100), "annees","nombre","Nb juv")
plot_states("Nad_ad[", 2, color[1],c(0,100), "annees","nombre","Nb ad")
